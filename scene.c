...
void update_transition(int delta_ms) {
    ...
    if (g_transition.fading_out) {
        ...
        if (g_transition.alpha >= 1.0f) {
            ...
            if (g_transition.on_complete) g_transition.on_complete(); // cambio de escena acá
        }
    } 
...
void draw_transition() {
    ...
    vert.x = 0; vert.y = 0; vert.z = 10; vert.argb = color; vert.oargb = 0;
    ...
    vert.x = 640; vert.y = 480; vert.flags = PVR_CMD_VERTEX_EOL;
    pvr_prim(&vert, sizeof(vert));
}
...
void scene_add_sprite(const char *name, Scene *scene, pvr_ptr_t tex, float x, float y, float w, float h,
                      ...
                      int fading_out, int fading_in, const char *file) {
...
void load_scene_from_json(Scene *scene, const char *filename) {
    ...
    if(!root) { printf("Error parseando JSON: %s\n", cJSON_GetErrorPtr()); free(json_data); return; }
...
if (cJSON_IsArray(sprites)) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (!cJSON_IsObject(spr)) continue;
        ...
        scene_add_sprite(name, scene, tex, x, y, w, h, tex_w, tex_h,
                         alpha, alpha_speed, fading_out, fading_in, file);
...
void draw_fade_overlay(float alpha) {
    ...
    vert.flags = PVR_CMD_VERTEX;
    vert.x = 0.0f;  vert.y = 0.0f;  vert.z = 10.0f;
    vert.u = vert.v = 0.0f;
    vert.argb = color;
    vert.oargb = 0;
    pvr_prim(&vert, sizeof(vert));
